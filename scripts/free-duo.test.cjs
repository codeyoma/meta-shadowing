...
test('normal prebuild strips stale free content and internal build requires explicit prepared package', t => {
  ...
  assert.throws(() => configureFreeDuo(result, { APPLE_FREE_DUO_TEST: '1', APPLE_BUILD_CHANNEL: 'internal', APPLE_ASSET_APP_GROUP: 'group.example.test' }, root), /Invalid free syntax metadata/);
  assert.deepEqual(result, {}, 'Rejected v1 metadata must not publish a replacement descriptor');
  ...
  fs.writeFileSync(path.join(root, 'private/free-duo-v2/manifest.json'), JSON.stringify({ ...manifest, version: 2, metadata: [] }));
  assert.throws(() => configureFreeDuo(result, { APPLE_FREE_DUO_TEST: '1', APPLE_BUILD_CHANNEL: 'internal', APPLE_ASSET_APP_GROUP: 'group.example.test' }, root));
  assert.deepEqual(result, {}, 'V2 without syntax must not publish a descriptor');
  fs.writeFileSync(path.join(root, 'private/free-duo-v2/manifest.json'), JSON.stringify({ ...manifest, version: 2 }));
