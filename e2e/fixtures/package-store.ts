...
export async function loadPackageModules(page: Page) {
  const result = buildSync({ stdin: { contents: 'import * as store from "./src/lib/lesson-package-store"; import * as downloader from "./src/lib/lesson-package-downloader"; import * as device from "./src/lib/device-learning-store"; import * as access from "./src/lib/device-access"; window.packageStore = store; window.packageDownloader = downloader; window.deviceStore = device; window.deviceAccess = access;', resolveDir: process.cwd() }, bundle: true, write: false, platform: "browser", format: "iife" });
