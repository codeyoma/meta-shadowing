// Short, valid Opus/WebM recording. The browser decodes and finishes real media.
export const testRecording = Buffer.from(
  "",
  "base64"
);

export function testAudioManifest(count: number) {
  return Array.from({ length: count }, (_, index) => ({
    phraseNumber: index + 1, canonicalName: `${index + 1}.webm`,
    path: `fixture/${index + 1}.webm`, contentType: "audio/webm", size: testRecording.length,
  }));
}
