...
- Preserve the selected mode for the extra pair: manual mode waits for Space/Continue between listens and for Space/Next (or R) after the fifth; automatic mode retains each level's speaking windows, then advances after the fifth speaking window and configured next-unit delay. Automatic mode still waits for the initial choice after three. Levels 6–8 keep their existing WPM and line timing.
