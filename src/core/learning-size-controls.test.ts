...
test('both real settings routes save immediately, share updates, reset, and recover from local failures', async t => {
  ...
  runtime.find('원문 크기 늘리기').onPress();
  assert.equal(settings.readSettings().originalTextSize, 21);
  assert.equal(runtime.find('원문 크기').value, '21');
  const plus = runtime.find('번역 크기 늘리기').onPress;
  plus(); plus(); plus();
  ...
  assert.equal(runtime.find('원문 크기').value, '21');
  runtime.find('원문 크기').onChangeText('48');
  assert.equal(runtime.find('원문 크기 늘리기').disabled, true, 'Bounds follow a valid draft before completion too');
  runtime.find('원문 크기').onEndEditing();
  assert.equal(settings.readSettings().originalTextSize, 48);
  assert.equal(runtime.find('원문 크기 늘리기').disabled, true);
  runtime.find('원문 크기').onChangeText('20');
  assert.equal(runtime.find('원문 크기 늘리기').disabled, false);
  runtime.find('원문 크기 늘리기').onPress();
  assert.equal(settings.readSettings().originalTextSize, 21);
  runtime.find('번역 크기').onChangeText('12');
  assert.equal(runtime.find('번역 크기 줄이기').disabled, true);
  runtime.find('번역 크기').onEndEditing();
  assert.equal(runtime.find('번역 크기 줄이기').disabled, true);
  fail = true;
  runtime.find('번역 크기 늘리기').onPress();
  assert.equal(runtime.find('번역 크기').value, '12');
  assert.equal(settings.readSettings().translationTextSize, 12);
  assert.deepEqual(alerts, ['설정을 저장하지 못했어요']);
  fail = false;
  runtime.find('글자 크기 초기화').onPress();
  assert.equal(runtime.find('원문 크기').value, '20');
  assert.equal(runtime.find('번역 크기').value, '18');
  runtime.render(null);
  params = { option: 'display' };
  runtime.render(React.createElement(SettingsScreen));
  assert.equal(runtime.find('원문 크기').value, '20');
  settings.saveSettings({ ...settings.readSettings(), originalTextSize: 35 }, 'guest', 0);
  assert.equal(runtime.find('원문 크기').value, '35', 'An already open editor follows external updates');
