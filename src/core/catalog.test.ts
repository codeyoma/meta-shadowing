...
test('all sixteen stage links resolve exactly and malformed links never start a different stage', () => {
  assert.equal(playableStage('1'), 1);
  assert.equal(playableStage('2'), 2);
  for (let stage = 1; stage <= 16; stage++) assert.equal(playableStage(String(stage)), stage);
  for (const param of ['17', '20', '0', '', undefined, ['1'], '01', '1.0', 1]) assert.equal(playableStage(param), null);
});
