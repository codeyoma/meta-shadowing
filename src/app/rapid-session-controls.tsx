...
function DelayControl({ label, value, onChange }: { label: string; value: number; onChange: (milliseconds: number) => void }) {
  ...
  return <Field>
    ...
    <Input id={id} type="number" min={0} max={30} step={0.5} value={value / 1000} onChange={event => {
...
export function RapidSessionControls({ level, settings, onChange }: { level: number; settings: RapidSettings; onChange: (settings: Partial<RapidSettings>) => void }) {
  ...
  return <FieldGroup>
    ...
    <Field>
      ...
      <NativeSelect id={`${id}-speed`} value={settings.wpmLevel} onChange={event => {
    ...
    <Field>
      ...
      <ToggleGroup type="single" variant="outline" value={settings.display} aria-labelledby={`${id}-display-label`} onValueChange={display => {
