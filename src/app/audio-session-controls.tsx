  ...
  return (
    <FieldGroup>
      <Field>
        ...
        <ToggleGroup type="single" variant="outline" value={settings.mode} aria-labelledby={`${id}-mode-label`} onValueChange={mode => {
      ...
      <Field>
        ...
        <NativeSelect id={`${id}-speed`} value={settings.playbackRate} onChange={event => onChange({ playbackRate: Number(event.target.value) })}>
      ...
      {settings.mode === "automatic" ? <Field>
        ...
        <Input id={`${id}-advance`} type="number" min={0} max={30} step={0.5} value={settings.advanceDelayMs / 1000} onChange={event => {
      ...
      {level === 4 || level === 5 ? <Field>
        ...
        <Input id={`${id}-group-gap`} type="number" min={0} max={30} step={0.5} value={(settings.groupGapMs ?? 500) / 1000} onChange={event => {
