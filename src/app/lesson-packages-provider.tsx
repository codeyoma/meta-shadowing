...
export function LessonPackagesProvider({ accountId, catalog, children }: { accountId: string; catalog: Lesson[]; children: ReactNode }) {
  ...
  const refresh = useCallback(() => {
    ...
    }).catch(() => { if (valid.current && epoch === readEpoch.current) { setLoading(false); setError(true); setInventory([]); } });
  }, [accountId]);
  ...
  const download = useCallback((lesson: Pick<Lesson, "id" | "version">) => {
    if (valid.current) void downloader.current?.download(lesson).catch(() => {}).finally(refresh);
  }, [refresh]);
...
export function PackageDownloads() {
  ...
  return <section className="mt-6 flex flex-col gap-4" aria-labelledby="package-downloads-title">
    ...
    {packages.error ? <Alert><AlertDescription>다운로드 저장 공간을 확인하지 못했습니다.<Button variant="outline" onClick={packages.refresh}>다운로드 다시 확인</Button></AlertDescription></Alert> : null}
