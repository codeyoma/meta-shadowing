...
import { saveLearningBoundary, type ProgressRecord, type CompletionRecord, type RunSelection } from "@/lib/learning-records";
...
export function useLearningRecord(lesson: Lesson, start: LearningStart) {
  ...
  const current = useRef({ active: false, anchor: 0, activeMs: start.progress?.activeMs ?? 0,
    settings: resolveSessionSettings(start.selection), completed: !!start.completion });
  ...
  const updateRecord = useCallback((update: RecordUpdate) => {
    ...
    if (update.settings) {
      ...
      saveLastSelection({ ...start.selection, ...state.settings });
      window.history.replaceState(null, "", getPlayerHref({ ...start.selection, ...state.settings }));
    }
    if (!update.checkpoint) return;
    const record: ProgressRecord = {
      runId: start.selection.runId, lessonId: lesson.id, lessonVersion: lesson.version, lessonName: lesson.name,
