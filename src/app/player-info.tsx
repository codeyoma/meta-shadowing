...
import { playableStage, isPlayableStage, isGroupedStage, isFirstWordStage } from '@/core/catalog';
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (unit) setResult({ key: contentKey, ...unit });
  ...
  return <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={{ padding: 24, gap: 20 }}>
    ...
    {!pack || !stage || (kind !== 'guide' && !analysis) ? <Label muted>학습 정보를 열 수 없어요.</Label>
      : analysis ? <>
        ...
        <Label muted>{isFirstWordStage(stage)
        ...
        <Label muted>음성이 모두 끝난 뒤 직접 확인해 주세요. 세 번 연습한 뒤 다음으로 이동하거나 두 번 더 반복할 수 있어요.</Label>
      </>}
  </ScrollView>;
}
