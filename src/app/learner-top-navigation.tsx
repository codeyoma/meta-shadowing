...
export function LearnerTopNavigation({ className }: { className?: string }) {
  ...
  const device = useDeviceJournal(account?.journal ?? { progress: null, history: [], studyDays: [] });
  const visibleStreak = account ? studyStreak(device.journal.studyDays) : 0;

  return <header className={cn(styles.homeHeader, className)}>
    <nav className={styles.homeNav} aria-label="상단 탐색">
      <Brand compact />
      {account && !account.loading && !device.loading ? <Badge variant="streak" aria-label={`${visibleStreak}일 연속 학습`} title="하루 한 번 이상 학습한 연속 일수">
