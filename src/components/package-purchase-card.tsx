...
export function PackagePurchaseCard({ section }: { section: 'owned' | 'store' }) {
  ...
  return <View style={{ gap: 14 }}>
    ...
    {issue && <Label size={14} muted>구매 내역을 검증하지 못했어요. 연결을 확인하고 설정의 구매 복원을 시도해 주세요. 이전에 확인한 구매가 취소되었다는 뜻은 아니에요.</Label>}
    ...
    {snapshot.outcome === 'pending' && <Label size={14} muted>승인이 완료되면 구매 내역이 자동으로 갱신돼요. 승인이 거절되었거나 만료되었다면 설정의 구매 복원으로 승인 상태를 다시 확인해 주세요.</Label>}
    {snapshot.outcome === 'restored' && snapshot.ownership === 'notOwned' && <Label size={14} muted>현재 App Store 계정에서 이 패키지의 구매 내역을 찾지 못했어요.</Label>}
    {snapshot.ownership === 'owned' && <ActionButton title="다운로드" icon="square.and.arrow.down" disabled onPress={() => {}} />}
